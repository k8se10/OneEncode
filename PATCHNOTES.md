...
Researched how third-party multistreaming tools (se.live/StreamElements et al.) actually get TikTok working, ahead of adding it as a real leg. Unlike Twitch/YouTube/Kick/Facebook, **TikTok LIVE has no open self-serve RTMP signup** — two real paths exist: (1) TikTok's own in-app "LIVE Studio"/"Cast to PC" feature, eligibility-gated and, critically, **issues a new RTMP server URL + stream key every broadcast session** rather than a stable long-lived credential; or (2) joining a **TikTok LIVE agency/Creator Network (MCN)**, an external business relationship that requests RTMP access on the creator's behalf — the mechanism se.live/StreamElements reportedly rely on. This is not a code-solvable problem — it's an account/business step. **User is pursuing the agency/MCN path.** Whether that path yields a static or per-session credential is unknown until obtained, so the config/UX design (does a TikTok leg fit the existing static-secret model, or does it need a "paste tonight's session key" dashboard action) is deliberately deferred rather than guessed at. Added a `TikTok LIVE` entry to `config/platformProfiles.yaml` (ingest spec only — H.264, 1080p30 @ 6000kbps default, 2s keyframe interval, CFR required) so the rendition groundwork is ready once a real credential exists; no leg/secret/UI work started. Full detail in CLAUDE.md/AGENTS.md's architecture decision #10 section.
